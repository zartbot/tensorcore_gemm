...
#define HGEMM_CHECK_CUDART_ERROR(_expr_)                                                          \
    do {                                                                                          \
        ...
        if (HGEMM_UNLIKELY(_ret_ != cudaSuccess)) {                                               \
            ...
            printf("CUDA Runtime API error = %04d \"%s\", runtime version: %d, driver version: %d", \
...
void launch_gemm(size_t M, size_t N, size_t K, half *A, half *B, half *C, half alpha, half beta)
{
    ...
    dim3 grid(BLOCK_STRIDE, CEIL_DIV(M, BLOCK_ROWS), CEIL_DIV(N, BLOCK_COLS * BLOCK_STRIDE));
